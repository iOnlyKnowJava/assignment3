...
int ut_socket(ut_socket_t *sock, const ut_socket_type_t socket_type,
  ...
  sock->recv_segs = NULL;
  sock->fin_wait_start_time.tv_sec = 0;
  sock->fin_wait_start_time.tv_nsec = 0;
...
int ut_close(ut_socket_t *sock) {
  ...
  if (sock != NULL) {
    ...
    while (sock->recv_segs != NULL) {
      recv_seg_t *seg = sock->recv_segs;
      sock->recv_segs = seg->next;
      free(seg);
    }
  } else {
...
int ut_write(ut_socket_t *sock, const void *buf, int length) {
  ...
  // Wait until complete_init becomes 1
  while (sock->complete_init == 0) {
