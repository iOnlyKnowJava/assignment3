...
typedef struct recv_seg {
  uint32_t start;
  uint32_t end;
  struct recv_seg *next;
} recv_seg_t;
...
typedef struct {
  ...
  recv_seg_t *recv_segs;  // linked list of out-of-order received segments
  struct timespec fin_wait_start_time; // Time when the socket starts waiting after receiving FIN and sending FIN
} ut_socket_t;
